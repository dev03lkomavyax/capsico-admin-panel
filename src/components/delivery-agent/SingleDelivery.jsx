  ...
  return (
    <div className="px-2 py-6 grid grid-cols-5 gap-4 items-center border-t border-[#C9C9C9]">
      <div className="flex gap-2 items-center">
        ...
        />
        <div>
          <h2 className="font-inter text-sm font-bold">
            {order?.restaurantId?.name || "N/A"}
          </h2>
          <p className="font-inter text-[10px] text-[#6B7280]">
            {order?.restaurantId?.address?.addressLine || "N/A"}
          </p>
        </div>
      </div>
      <p className="text-xs font-inter text-[#6B7280] text-center">
        {order?.timing?.orderedAt &&
          format(new Date(order?.timing?.orderedAt), "MMM dd, hh:mm a")}
      </p>
      <p className="text-sm font-inter font-bold text-center text-[#363636]">
        {order?.amounts?.total ? `₹ ${order?.amounts?.total}` : "N/A"}
      </p>

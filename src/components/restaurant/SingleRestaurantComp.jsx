...
import { isRestaurantOpen } from '@/utils/resturantopenclose' // adjust path if needed

const SingleRestaurantComp = ({ data, getAllRestaurant }) => {
  const navigate = useNavigate()
  const [isAlertModalOpen, setIsAlertModalOpen] = useState(false)

  // Get times (adjust the property if your API is different)
  const openingTime = data?.timings?.openTime || "10:00"
  const closingTime = data?.timings?.closeTime || "23:00"

  // Real-time open/close calculation using local time
  const [isOpenRealtime, setIsOpenRealtime] = useState(false)
  useEffect(() => {
    function checkStatus() {
      setIsOpenRealtime(isRestaurantOpen(openingTime, closingTime))
    }
    checkStatus()
  ...
  return (
    <>
      <TableRow>
        ...
        {/* Real-time open/close status (with your toggle button and TIME BELOW! */}
        <TableCell className="font-sans">
          <StatusToggleButton
            active={isOpenRealtime}
            loading={false}
            onClick={undefined} // No manual toggling, status is automatic
            activeLabel="Open"
            inactiveLabel="Closed"
            className="mb-1"
          />

...
const DeliveryPartnerReq = ({ req, refetch }) => {
  ...
  const updateStatus = () => {
    fetchData(`/admin/updateStatus/${req?._id}`, {
      status: "approved",
    });
  };
  ...
  const deleteReq = () => {
    fetchData1(
      `/admin/delete-delivery-agent?deliveryAgentId=${req?._id}`
    );
  };
    ...
    <>
      <TableRow>
        ...
        <TableCell className="text-[#1D1929] text-xs font-bold capitalize font-sans">
          {req?.personalInfo?.name}
        </TableCell>
        <TableCell className="text-[#1D1929] text-xs font-sans">
          {req?.personalInfo?.email}
        </TableCell>
        <TableCell className="text-[#1D1929] text-xs font-sans">
          {req?.personalInfo?.phone}
        </TableCell>
        ...
        <TableCell className="text-[#1D1929] text-xs capitalize font-normal font-roboto">
          {/* <Badge variant={"outline"}>{req?.accountStatus?.currentStatus}</Badge> */}
          <div
            className={`${statusStyles[req?.accountStatus?.currentStatus].bg} ${
              statusStyles[req?.accountStatus?.currentStatus].text
            } w-24 flex justify-center capitalize items-center h-[24px] text-[10px] font-normal font-sans rounded-[10px]`}
          >
            {req?.accountStatus?.currentStatus}
          </div>
        </TableCell>
        <TableCell>
...
export default DeliveryPartnerReq;

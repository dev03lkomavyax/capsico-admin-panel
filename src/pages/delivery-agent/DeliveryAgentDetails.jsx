...
const DeliveryAgentDetails = () => {
    ...
    const [deliveryPartnerDetailsData, setDeliveryPartnerDetailsData] = useState("");
            ...
            <section>
                <div className='flex justify-between h-14'>
                    <button onClick={() => navigate(-1)} className="flex items-center gap-1">
                        ...
                        <span className='font-roboto text-lg font-medium'>Delivery agent detail</span>
                    </button>
                </div>
                <div className="grid grid-cols-[60%_38%] gap-[2%]">
                    <div className='bg-white p-5 rounded-lg flex gap-10'>
                        <img className='w-52 h-52 rounded-lg' src={deliveryPartnerDetailsData?.personalInfo?.profileImage} alt="avatar" />
                        <div className='w-full'>
                            <div className='flex w-full justify-between items-center gap-3'>
                                <h1 className='font-inter text-3xl font-semibold text-[#202020]'>{deliveryPartnerDetailsData?.personalInfo?.name}</h1>
                                <img onClick={() => navigate("/admin/delivery-agent/123/edit-profile")} className='w-5 h-5 cursor-pointer' src={edit} alt="edit" />
                            </div>
                            <div className="flex flex-col gap-3 mt-5">
                                <div className="flex items-center gap-2">
                                    <img className='w-5 h-5' src={location} alt="location" />
                                    <p className='font-inter text-[#696969] text-lg'>{deliveryPartnerDetailsData?.address?.city}</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <img className='w-5 h-5' src={sms} alt="sms" />
                                    <p className='font-inter text-[#696969] text-lg'>{deliveryPartnerDetailsData?.personalInfo?.email}</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <img className='w-5 h-5' src={call} alt="call" />
                                    <p className='font-inter text-[#696969] text-lg'>+91 {deliveryPartnerDetailsData?.personalInfo?.phone}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div onClick={() => navigate("/admin/delivery-agent/123/available-balance")} className='flex flex-col cursor-pointer justify-between rounded-lg bg-[#7FADFF]'>
                ...
                <div className="rounded-lg mt-7 p-5 bg-white">
                    ...
                    <div className="flex flex-col mt-5">
                        <SingleDelivery rating={true} />
                        <SingleDelivery rating={false} />
                        <SingleDelivery rating={true} />
                    </div>
                </div>
            </section>
